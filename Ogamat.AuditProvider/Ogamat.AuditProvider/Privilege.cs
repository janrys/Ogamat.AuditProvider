...
using Luid = Ogamat.AuditProvider.NativeMethods.LUID;
...
namespace Ogamat.AuditProvider
{
    ...
    public delegate void PrivilegedHelper();
                    ...
                    if (false == NativeMethods.LookupPrivilegeValue(null, privilege, ref luid))
                    {
                        ...
                        else if (error == NativeMethods.ERROR_NO_SUCH_PRIVILEGE)
                        {
                            throw new ArgumentException(
                                string.Format("{0} is not a valid privilege name", privilege), nameof(privilege));
                        }
                        else
                        {
